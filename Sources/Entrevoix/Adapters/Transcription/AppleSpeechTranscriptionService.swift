...
actor AppleSpeechResourceManager {
    private var reservedLocale: Locale?
    private(set) var state: ProviderPreparationState = .checking

    func preparationState(for requestedLocale: Locale) async -> ProviderPreparationState {
    ...
    func reserve(locale requestedLocale: Locale) async throws -> Locale {
        guard let locale = await SpeechTranscriber.supportedLocale(equivalentTo: requestedLocale) else { throw AppleProviderError(capability: .stt, reason: .unsupportedLocale) }
...
struct AppleSpeechTranscriptionService: SpeechTranscribing {
    let resources: AppleSpeechResourceManager
