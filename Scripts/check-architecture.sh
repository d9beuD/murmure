...
if rg -n "$forbidden_core_imports" Sources/EntrevoixCore; then
    echo "EntrevoixCore must not import UI, platform, or technical frameworks." >&2
    exit 1
fi
...
violations="$(rg -l "$adapter_construction" Sources/Entrevoix --glob '*.swift' | grep -v '^Sources/Entrevoix/App/CompositionRoot.swift$' || true)"
