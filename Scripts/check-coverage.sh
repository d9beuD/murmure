    ...
    jq -r \
        ...
        | select(
            if $group == "core" then
                ...
                .filename | test("/Sources/Murmure/(App/AppModel|Features/Settings/ConnectionTestModel|Infrastructure/(Accessibility/FocusedTextElementResolver|Delivery/TextDelivery|Persistence/(KeychainStore|UserDefaultsPreferencesStore)|Cleanup/OpenAITextCleanupService|Transcription/OpenAITranscriptionService|Networking/SafeNetworkSession))\\.swift$")
            end
        )
